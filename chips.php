...
<?php
    ...
    //leg verbinding met de database
        ...
        $query = $db->prepare("SELECT product.brand, product.prodname, product.proddesc, product.imageref FROM `type` INNER JOIN product ON type.idtype=product.typeid WHERE `idtype` = '3'");
        $query->execute();
